...
function saveUser($status, $array) {
	...
	if ($status == 'activation') {
		...
		if ($array['email']) {
			$array['email'] = dataclear($array['email'], 'nospaces code');
		}
		if ($array['verify']) {
			$array['verify'] = dataclear($array['verify'], 'nospaces');
		}
...
function dbUser($status, $array) {
	...
	foreach ($array as $key => &$item) {
		if (in_array($key, $clear)) {
			if ($key === 'phone') {
				$item = dataclear($item, 'nospaces phone');
			} else {
				$item = dataclear($item, 'nospaces code');
			}
		}
	}
...
function saveCard($status, $PID, $data) {
	...
	foreach ($content as &$item) {
		$item['content'] = dataclear($item['content'], 'html');
	}
