...
function dbSelect($db, $name) {
	...
	$base = [
		'structures' => [
			'name' => PATH_ADMINISTRATOR . DIRECTORY_SEPARATOR . $name . '.ini',
			'json' => true,
			'format' => 'structure'
		],
		'settings' => [
		...
		'templates' => [
			'name' => PATH_TEMPLATES . DIRECTORY_SEPARATOR . $name . DIRECTORY_SEPARATOR . 'settings.ini',
			'json' => true,
			'format' => false
		]
	];
...
function dbUse($db, $query, $array = false, $where = false, $options = false) {
	...
	if (!$query || !in_array($query, ['select', 'count', 'insert', 'update', 'delete', 'verify', 'connect'])) { return false; }
	...
	if ($db -> name === 'languages') {
		$db -> where['name'] = PATH_TEMPLATES . DIRECTORY_SEPARATOR . $db -> where['template'] . DIRECTORY_SEPARATOR . 'lang' . DIRECTORY_SEPARATOR . $db -> where['name'];
		if ($db -> array[0] === 'lang') {
			$db -> where['name'] .= '.lng';
		...
		$db -> where['name'] = PATH_MODULES . DIRECTORY_SEPARATOR . $db -> where['name'] . DIRECTORY_SEPARATOR . 'data' . DIRECTORY_SEPARATOR . ($db -> where['param'] ? $db -> where['param'] : 'settings') . '.ini';
	} elseif ($db -> options['add']['folder'] && $db -> name === 'articles_' . $db -> options['add']['folder']) {
		$db -> where['name'] = PATH_ARTICLES . DIRECTORY_SEPARATOR . $db -> options['add']['folder'] . DIRECTORY_SEPARATOR . $db -> where['name'] . '.' . $db -> options['add']['ext'];
	}
	...
	if ($db -> options['json']) {
		...
		$db -> name = PATH_ADMINISTRATOR . DIRECTORY_SEPARATOR . $db -> name . '.csv';
