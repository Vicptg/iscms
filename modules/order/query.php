...
$module -> data = dataloadcsv($module -> base -> path . DIRECTORY_SEPARATOR . $module -> base -> name);
...
if (!$module -> status) {
	...
	if (strpos($module -> form -> place, ',') !== false) {
		...
		$module -> form -> places = array_diff($module -> form -> places, array(''));
		...
		foreach ($module -> form -> places as $place) {
			...
			$module -> data[] = [(string)strtotime($module -> form -> date . ' ' . $module -> form -> time . ' UTC'), $sort[3][0], $sort[3][1], $sort[3][2], $module -> form -> email, '{user: ' . $module -> form -> user . ', phone: ' . $module -> form -> phone . ', ip: ' . $_SERVER['REMOTE_ADDR'] . ', browser: ' . $_SERVER['HTTP_USER_AGENT'] . ' lang: ' . $module -> form -> lang . '}', $module -> form -> currdate];
		...
		$module -> data[] = [(string)strtotime($module -> form -> date . ' ' . $module -> form -> time . ' UTC'), $sort[0][0], $sort[0][1], $sort[0][2], $module -> form -> email, '{user: ' . $module -> form -> user . ', phone: ' . $module -> form -> phone . ', ip: ' . $_SERVER['REMOTE_ADDR'] . ', browser: ' . $_SERVER['HTTP_USER_AGENT'] . ' lang: ' . $module -> form -> lang . '}', $module -> form -> currdate];
		
	}
	
	rsort($module -> data);
	
	datasavecsv($module -> data, $module -> base -> path . DIRECTORY_SEPARATOR . $module -> base -> name);
	...
	message($module -> send, $module -> lang -> subject, (object) array('value' => $module -> original, 'label' => $module -> lang), $module -> lang -> text, $module -> base -> path . DIRECTORY_SEPARATOR . $module -> base -> name . '_error');
