...
<?php if ($module -> settings -> logo || $module -> settings -> title) : ?>
	...
	<a
		href="<?= $template -> url; ?><?= ($template -> lang !== ROOT_LANG) ? '/' . $template -> lang : '' ; ?>/"
		class="
			<?= $module -> param; ?>_brand
	...
	>
		...
		<?php else : ?>
			...
			<img
				src="<?php
					echo $template -> url . '/' . NAME_UPLOAD . '/logo.';
					if (file_exists(PATH_UPLOAD . DIRECTORY_SEPARATOR . 'logo.svg')) { echo 'svg'; }
					elseif (file_exists(PATH_UPLOAD . DIRECTORY_SEPARATOR . 'logo.png')) { echo 'png'; }
					else { echo 'jpg'; }
				?>"
				alt="<?= $lang -> title; ?>"
				title="<?= $lang -> title; ?>"
